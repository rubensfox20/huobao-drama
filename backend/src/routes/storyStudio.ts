...
app.post('/episode-scripts', async (c) => {
  ...
  try {
    ...
    const requested = new Set(input.episodes.map(episode => episode.episode_number))
    if (result.episodes.length !== requested.size || result.episodes.some(episode => !requested.has(episode.episode_number))) {
      throw new Error('A IA não retornou todos os roteiros solicitados')
    }
    return success(c, result)
  } catch (error) {
